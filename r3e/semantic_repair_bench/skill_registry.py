"""R³E skill registry: 经验沉淀为 conditional policy(非 prompt 注入).
...
def route(case: dict, registry: list[dict]) -> tuple[dict, str]:
    ...
    promoted = [s for s in registry if s.get("status") == "promoted"]
    # 具体 family 匹配优先
    for sk in promoted:
